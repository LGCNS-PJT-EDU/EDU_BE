...
public class ExamLevelCalculator {
  ...
  public int calculateScorePercent(List<ExamAnswerDto> answers) {
    int score = answers.stream().mapToInt(a -> calculateScoreByDifficulty(a, false)).sum();
    int maxScore = answers.stream().mapToInt(a -> calculateScoreByDifficulty(a, true)).sum();
