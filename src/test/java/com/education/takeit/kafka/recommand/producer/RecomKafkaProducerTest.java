...
class RecomKafkaProducerTest {
    ...
    @Mock
    private KafkaTemplate<String, RecomRequestDto> recomKafkaTemplate;

    @Mock
    private KafkaTemplate<String, RecomFailDto> recomFailKafkaTemplate;
    ...
    void 정상_발행_성공() {
        ...
        ProducerRecord<String, RecomRequestDto> producerRecord =
                new ProducerRecord<>(RECOM_REQUEST_TOPIC, testRequestDto.userId().toString(), testRequestDto);
        RecordMetadata metadata = new RecordMetadata(
                new TopicPartition(RECOM_REQUEST_TOPIC, 0),
                0L, 0, System.currentTimeMillis(), 0, 0
        );
        SendResult<String, RecomRequestDto> sendResult =
        ...
        //Then
        verify(recomKafkaTemplate, times(1)).send(
                eq(RECOM_REQUEST_TOPIC),
                eq(testRequestDto.userId().toString()),
                any(RecomRequestDto.class) // any() 매처 사용
        );

        verify(recomFailKafkaTemplate, never())
                .send(anyString(), anyString(), any(RecomFailDto.class));
    ...
    void 발행_실패_실패_토픽_전송() {
        ...
        verify(recomFailKafkaTemplate, timeout(100).times(1))
                .send(eq(RECOM_FAIL_TOPIC), eq(testRequestDto.userId().toString()), failDtoCaptor.capture());
