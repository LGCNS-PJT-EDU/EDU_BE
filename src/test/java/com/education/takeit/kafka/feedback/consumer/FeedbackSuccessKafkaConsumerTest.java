...
class FeedbackSuccessKafkaConsumerTest {
    ...
    @InjectMocks
    private FeedbackSuccessKafkaConsumer consumer;
    ...
    void setUp() {
        ...
        testPayload = new FeedbackResultDto(
                1L,
                1L,
                "pre",
                1,
    ...
    @Test
    @DisplayName("성공적인 피드백 수신 시 FeedbackService.saveFeedback이 호출되고 Acknowledgment.acknowledge()가 호출되어야 한다")
    ...
    @Test
    @DisplayName("피드백 저장 실패 시 FeedbackService.saveFeedback에서 예외가 발생하고 Acknowledgment.acknowledge()가 호출되지 않아야 한다")
