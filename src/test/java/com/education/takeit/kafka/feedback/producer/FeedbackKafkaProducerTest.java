...
class FeedbackKafkaProducerTest {
    ...
    @Mock
    private KafkaTemplate<String, FeedbackRequestDto> feedbackKafkaTemplate;

    @Mock
    private KafkaTemplate<String, FeedbackFailDto> feedbackFailKafkaTemplate;
    ...
    void 정상_발행_성공() {
        ...
        ProducerRecord<String, FeedbackRequestDto> producerRecord =
                new ProducerRecord<>(FEEDBACK_REQUEST_TOPIC, testRequestDto.userId().toString(), testRequestDto);
        RecordMetadata metadata = new RecordMetadata(
                new TopicPartition(FEEDBACK_REQUEST_TOPIC, 0),
                0L, 0, System.currentTimeMillis(), 0, 0
        );
        SendResult<String, FeedbackRequestDto> sendResult =
        ...
        // Then
        verify(feedbackKafkaTemplate, times(1)).send(
                eq(FEEDBACK_REQUEST_TOPIC),
                eq(testRequestDto.userId().toString()),
                any(FeedbackRequestDto.class)
        );
    ...
    void 정상_발행_실패_시_트리거_검증() {
        ...
        CompletableFuture<SendResult<String, FeedbackRequestDto>> failedFuture = new CompletableFuture<>();
        ...
        verify(feedbackFailKafkaTemplate, timeout(100).times(1))
                .send(eq(FEEDBACK_FAIL_TOPIC), eq(testRequestDto.userId().toString()), failDtoCaptor.capture());
