...
class RecomSuccessKafkaConsumerTest {
  ...
  @InjectMocks private RecomSuccessKafkaConsumer consumer;

  private RecomResultDto testPayload;

  @BeforeEach
  void setUp() {
    UserContentResDto content1 =
        new UserContentResDto(
            ...
            true,
            "Good intro");
    UserContentResDto content2 =
        new UserContentResDto(
            1002L,
            201L,
            "Spring Kafka Deep Dive",
            "http://example.com/spring",
            "article",
            "Blog",
            "30min",
            "Paid",
    ...
    testPayload = new RecomResultDto(1L, 1L, Arrays.asList(content1, content2));
  }

  @Test
  @DisplayName(
      "성공적인 추천 컨텐츠 수신 시 RecommendService.saveUserContents가 호출되고 Acknowledgment.acknowledge()가 호출되어야 한다")
  ...
  @Test
  @DisplayName(
      "추천 컨텐츠 저장 실패 시 RecommendService.saveUserContents에서 예외가 발생하고 Acknowledgment.acknowledge()가 호출되지 않아야 한다")
