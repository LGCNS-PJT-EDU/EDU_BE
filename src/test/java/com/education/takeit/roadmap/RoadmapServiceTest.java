...
public class RoadmapServiceTest {
    ...
    void setUp() {
        ...
        RoadmapManagement FE_roadmapManagement = RoadmapManagement.builder()
        ...
        RoadmapManagement BE_roadmapManagement = RoadmapManagement.builder()
        ...
        FE_roadmapList = List.of(
                Roadmap.builder().roadmapId(1L).orderSub(1).subject(subject).roadmapManagement(FE_roadmapManagement).build(),
                Roadmap.builder().roadmapId(2L).orderSub(2).subject(subject).roadmapManagement(FE_roadmapManagement).build(),
                Roadmap.builder().roadmapId(3L).orderSub(3).subject(subject).roadmapManagement(FE_roadmapManagement).build(),
                // ... 생략
                Roadmap.builder().roadmapId(26L).orderSub(26).subject(subject).roadmapManagement(FE_roadmapManagement).build()
        );

        // BE 로드맵 데이터 (roadmap_management_id == 2)
        BE_roadmapList = List.of(
                Roadmap.builder().roadmapId(27L).orderSub(1).subject(subject).roadmapManagement(BE_roadmapManagement).build(),
                Roadmap.builder().roadmapId(28L).orderSub(2).subject(subject).roadmapManagement(BE_roadmapManagement).build(),
                Roadmap.builder().roadmapId(29L).orderSub(3).subject(subject).roadmapManagement(BE_roadmapManagement).build(),
                // ... 생략
                Roadmap.builder().roadmapId(37L).orderSub(11).subject(subject).roadmapManagement(BE_roadmapManagement).build()
        );
    ...
    void 비회원의_진단_결과로_로드맵_생성() throws JsonProcessingException {
        // given
        List<DiagnosisAnswerRequest> answers = List.of(
                new DiagnosisAnswerRequest(1L, "FE")
        );

        List<Subject> essentialSubjects = List.of(
        ...
        when(objectMapper.writeValueAsString(answers)).thenReturn("[{\"questionId\":1,\"answer\":\"FE\"}]");
        ...
        verify(valueOperations).set(matches("guest:.*:subjects"), anyString(), eq(Duration.ofMinutes(15)));
        verify(valueOperations).set(matches("guest:.*:answers"), anyString(), eq(Duration.ofMinutes(15)));
    ...
    void 회원의_진단_결과로_로드맵_생성(){
        ...
        RoadmapSaveResDto roadmapSaveResDto = new RoadmapSaveResDto(
                null, 1L, subjects
        );
    ...
    void 로드맵_생성(){
        ...
        // React 과목들
        when(subjectRepository.findById(10L)).thenReturn(Optional.of(new Subject(10L, "React", "FE", "N", 3, "", track)));
        when(subjectRepository.findById(11L)).thenReturn(Optional.of(new Subject(11L, "Redux", "FE", "N", 4, "", track)));
        when(subjectRepository.findById(12L)).thenReturn(Optional.of(new Subject(12L, "Zustand", "FE", "N", 5, "", track)));
        when(subjectRepository.findById(22L)).thenReturn(Optional.of(new Subject(22L, "Next.js", "FE", "N", 6, "", track)));
        when(subjectRepository.findById(23L)).thenReturn(Optional.of(new Subject(23L, "React 랜더링", "FE", "N", 7, "", track)));
        when(subjectRepository.findById(24L)).thenReturn(Optional.of(new Subject(24L, "React Query 심화", "FE", "N", 8, "", track)));

        // 기타 조건 과목들
        when(subjectRepository.findById(21L)).thenReturn(Optional.of(new Subject(21L, "EsLint & Prettier", "FE", "N", 9, "", track)));
        when(subjectRepository.findById(17L)).thenReturn(Optional.of(new Subject(17L, "Storybook", "FE", "N", 10, "", track)));
        when(subjectRepository.findById(18L)).thenReturn(Optional.of(new Subject(18L, "Tailwind CSS", "FE", "N", 11, "", track)));
        when(subjectRepository.findById(19L)).thenReturn(Optional.of(new Subject(19L, "SCSS", "FE", "N", 12, "", track)));
        when(subjectRepository.findById(20L)).thenReturn(Optional.of(new Subject(20L, "styled", "FE", "N", 13, "", track)));
        when(subjectRepository.findById(33L)).thenReturn(Optional.of(new Subject(33L, "Component Test", "FE", "N", 14, "", track)));

        // When
        RoadmapSaveResDto result = roadmapService.createRoadmap(answers);
        assertThat(result).isNotNull();
        // Then
        List<Long> actualSubjectIds = result.subjects().stream()
                .map(SubjectDto::subjectId)
                .toList();

        List<Long> expectedSubjectIds = List.of(
                1L, 2L, 3L, 10L, 4L, 11L, 5L, 12L,
                6L, 22L, 7L, 23L, 8L, 24L, 9L,
                21L, 17L, 18L, 19L, 20L, 33L, 29L
        );
    ...
    void 사용자_로드맵_저장() throws JsonProcessingException {
        ...
        verify(user).updatePreferences(
                eq(LectureAmount.values()[1]),
                eq(PriceLevel.values()[2]),
                eq(true)
        );
        verify(roadmapRepository).saveAll(argThat(roadmaps -> {
    ...
    """;
        ...
        List<DiagnosisAnswerRequest> parsedAnswers = List.of(
        ...
        when(redisTemplate.opsForValue().get("guest:" + uuid + ":subjects")).thenReturn(redisSubjectIds);
        when(redisTemplate.opsForValue().get("guest:" + uuid + ":answers")).thenReturn(redisAnswersJson);
    ...
    void 로드맵_진척도_조회(){
        //Given
        RoadmapManagement roadmapManagement = RoadmapManagement.builder()
    ...
    void 로드맵_수정(){
        ...
        List<SubjectDto> updatedSubjects = List.of(
                new SubjectDto(1L, "HTML", 10),
                new SubjectDto(3L, "JavaScript", 11)
        );
        ...
        // subject2는 제거되어야 함
        verify(roadmapRepository).deleteAll(argThat(toDelete -> {
            List<Roadmap> list = (List<Roadmap>) StreamSupport.stream(toDelete.spliterator(), false).toList();
            return list.size() == 1 &&
                    list.get(0).getSubject().getSubId().equals(2L);
        }));

        verify(roadmapRepository).saveAll(argThat(toSave -> {
            List<Roadmap> list = StreamSupport.stream(toSave.spliterator(), false).toList();
            return list.size() == 2 &&
                    list.stream().anyMatch(r -> r.getSubject().getSubId().equals(1L) && r.getOrderSub() == 10) &&
                    list.stream().anyMatch(r -> r.getSubject().getSubId().equals(3L) && r.getOrderSub() == 11);
        }));
    }
    ...
    void FE_기본_로드맵_저장() {
        ...
        when(roadmapManagementRepository.findByUserId(userId)).thenReturn(mock(RoadmapManagement.class));
        when(roadmapRepository.findByRoadmapManagement_RoadmapManagementId(roadmapManagementId)).thenReturn(defaultRoadmaps);
        ...
        doReturn(new RoadmapFindResDto(subjectDtos, "FE_DEFAULT_ROADMAP", userLocationSubjectId))
                .when(roadmapService).getDefaultRoadmap(roadmapType);
    ...
    void 사용자_로드맵_반환_로드맵_없음() {
        // Given
        RoadmapManagement roadmapManagement = RoadmapManagement.builder()
                .roadmapManagementId(20L)
                .roadmapNm("Empty Roadmap")
                .build();

        when(roadmapRepository.findByRoadmapManagement_RoadmapManagementId(20L))
                .thenReturn(List.of());
    ...
    void 로드맵_반환_요청시_분기_처리_회원_로드맵_없고_uuid_있음() {
        ...
        RoadmapSaveResDto saved = new RoadmapSaveResDto(uuid, 1L, List.of(
                new SubjectDto(1L, "HTML", 1)
        ));
    ...
    void 로드맵_반환_요청시_분기_처리_비회원_uuid_있음() {
        ...
        RoadmapFindResDto expected = new RoadmapFindResDto(
                List.of(new SubjectDto(1L, "CSS", 1)), "guest-uuid", 1L);
    ...
    void 사용자_로드맵의_과목_데이터_반환() {
        ...
        Chapter chapter2 =
                Chapter.builder()
                        .chapterName("텍스트 & 목록요소")
                        .chapterOrder(2)
                        .subject(subject)
                        .build();

        List<UserContentResDto> recommendations = List.of(
                new UserContentResDto(1L, subjectId, "제목1", "https://example.com", "유형1", "", "", "", true, ""),
                new UserContentResDto(2L, subjectId, "제목2", "https://example2.com", "유형2", "", "", "", false, "")
        );
        ...
        when(roadmapRepository.findBySubjectAndRoadmapManagement(subject, roadmapManagement))
                .thenReturn(roadmap);
        when(recommendService.findRecommendations(userId, subjectId))
